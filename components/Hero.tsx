  ...
  return (
    <div className="relative pt-32 pb-20 sm:pt-40 sm:pb-32 text-center">
      ...
      <p className="text-xl md:text-2xl text-indigo-100/70 max-w-2xl mx-auto mb-12 leading-relaxed font-light">
        The most advanced adaptive testing platform. 
        Analyze your performance, identify weak spots, and improve your rank with AI-driven insights.
      </p>
      
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-20 px-4">
        <button className="w-full sm:w-auto px-8 py-4 bg-white text-indigo-900 rounded-full font-bold hover:bg-gray-50 transition-all transform hover:scale-105 shadow-[0_0_40px_rgba(255,255,255,0.2)] flex items-center justify-center gap-2 text-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-5xl mx-auto border-t border-white/5 pt-12 px-4">
        {[
          ...
          <div key={i} className="text-center group cursor-default">
            <div className="text-3xl md:text-4xl font-bold text-white mb-1 group-hover:text-indigo-300 transition-colors">{stat.value}</div>
            <div className="text-sm md:text-base text-indigo-200/50 font-medium group-hover:text-indigo-200/80 transition-colors">{stat.label}</div>
          </div>
        ))}
      </div>
    </div>
